...
export default function Signup() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true)
    triggerLoader('Registering Operator…', 'Creating credentials in Supabase Auth', 700)
    ...
    try {
      ...
      if (error) {
        ...
        if (data?.session) {
          triggerLoader('Signing In…', 'Session established, entering TrackWave portal', 500)
          navigate('/', { replace: true })
        } else {
          setSuccessMsg(
            'Registration successful! Please check your email for confirmation, or proceed to sign in.'
          )
          setTimeout(() => {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 px-4 py-12 text-slate-100 sm:px-6 lg:px-8">
      ...
      <div className="pointer-events-none fixed inset-0 flex items-center justify-center overflow-hidden opacity-20">
        <div
          className="h-[600px] w-[600px] rounded-full border border-indigo-500/20 animate-ping"
          style={{ animationDuration: '7s' }}
        />
        <div className="absolute h-[800px] w-[800px] rounded-full border border-slate-800" />
      </div>

      <div className="relative w-full max-w-md space-y-7 rounded-2xl border border-slate-800/80 bg-slate-900/60 p-8 shadow-2xl backdrop-blur-xl">
        {/* Header with Logo */}
        <div className="text-center">
          <div className="mx-auto mb-4 flex items-center justify-center">
            <div className="relative flex items-center justify-center">
              <div className="absolute -inset-2 animate-pulse rounded-full bg-gradient-to-r from-cyan-500 via-indigo-500 to-emerald-500 opacity-60 blur-md" />
              <img
                src="/logo.png"
                alt="TrackWave Logo"
          ...
          <p className="mt-1.5 text-xs text-slate-400">
            Register for TrackWave railway operations, predictive dispatch & twin telemetry
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-xs text-slate-400">
            By registering, you confirm you are an authorized railway operations personnel.
          </div>
